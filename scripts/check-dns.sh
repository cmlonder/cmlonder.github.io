...
echo "$body" | grep -q "Hashnode" && no "anasayfada hâlâ Hashnode izi var" || ok "Hashnode servis etmiyor"
echo "$body" | grep -q "Workbench" && ok "yeni site servis ediliyor" || no "yeni site GÖRÜNMÜYOR"
